...
3. **初回はダブルクリックでは開けない。**署名が ad-hoc で公証もしていないため Gatekeeper に止められ、
   macOS 15(Sequoia)では**「“Mihari.app” は壊れているため開けません。ゴミ箱に入れる必要があります。」**
   というダイアログが出る。**アプリは壊れていない。**ダウンロードに付く検疫属性
   (`com.apple.quarantine`)と ad-hoc 署名の組み合わせでこの文言になるだけで、zip が無傷なことは
   チェックサムで確かめられる。ゴミ箱には入れず、次のどちらかで検疫属性を外す。

   ```sh
   # 展開してアプリケーションフォルダへ移したあとに実行する
   xattr -dr com.apple.quarantine /Applications/Mihari.app

   # または、展開する前に zip に対して実行する(こうすると展開後の .app に検疫属性が付かない)
   xattr -d com.apple.quarantine ~/Downloads/Mihari-<バージョン>.zip
   ```

   これは**ダウンロード時に付いた検疫フラグを外す**操作、つまり**入手元を自分で信頼すると
   macOS に宣言する**操作である。だから zip は必ず上記の公式 Releases から取得し、
   `shasum -a 256 Mihari-<バージョン>.zip` の出力がリリースページのチェックサムと
   一致することを確かめてから実行すること。

   なお、**ad-hoc 署名では「右クリック →「開く」」も、システム設定 → プライバシーとセキュリティの
   「このまま開く」も出ないことが多い**(「壊れている」扱いのため)。出た場合はそれで開いてもよい。
4. 検疫属性を外したあとは、ふつうにダブルクリックで起動する
